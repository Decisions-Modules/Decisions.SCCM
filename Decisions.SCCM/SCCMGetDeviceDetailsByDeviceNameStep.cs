...
namespace SCCM_2012
{
    [AutoRegisterStep("Get Device Details By Device Name", "SCCM Steps", "Device")]
    //[AutoRegisterAgentFlowElementStep("Get Device Details By Device Name", "SCCM Steps", "Device")]
    [Writable]
    public class SCCMGetDeviceDetailsByDeviceNameStep : SCCMBaseStep, ISyncStep, IDataConsumer, IDataProducer
    {
        ...
        public ResultData Run(StepStartData data)
        {
            ...
            string deviceName = data.Data["Device Name"] as string;

            try
            {
                string queryFromWhere = string.Format("FROM SMS_R_System WHERE Name = '{0}'", deviceName);
                ...
                {
                    throw new Exception(string.Format("More than one device was found by Device Name '{0}'", deviceName));
                }
        ...
        {
            get { return new DataDescription[] { new DataDescription(new DecisionsNativeType(typeof(string)), "Device Name") }; }
        }
    }
}
